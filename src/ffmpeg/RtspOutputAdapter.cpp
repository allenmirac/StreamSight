...
#include "xop/media.h"
#include "xop/SeiLatencyMarker.h"
#include <cstring>
#include <cstdio>
#include <iostream>
#include <chrono>
...
bool RtspOutputAdapter::WritePacket(const AVPacket* pkt) {
	...
	if (enable_latency_sei_ && is_key) {
		...
		uint64_t send_time_us = xop::GetSendTimeUs();
		auto sei_nalu = xop::BuildLatencySeiNalu(
		    (uint64_t)frame_count_, send_time_us, encode_us);

		xop::AVFrame sei_frame((uint32_t)sei_nalu.size());
		std::memcpy(sei_frame.buffer.get(), sei_nalu.data(), sei_nalu.size());
		sei_frame.type = xop::VIDEO_FRAME_I;
		sei_frame.timestamp = xop::H264Source::GetTimestamp();
